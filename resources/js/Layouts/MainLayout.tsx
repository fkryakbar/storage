...
export default function MainLayout({ auth, children }: { children: React.ReactNode, auth: { user: User } }) {
    ...
    return (
        <>
            ...
            <div className="flex lg:w-[60%] w-[90%] mx-auto justify-center lg:mt-20 mt-10 gap-3">
                <div className="basis-[20%] lg:block hidden">
                    <Menu name={auth.user.name} />
                </div>
                <div className="lg:basis-[80%] w-[95%] mx-auto">
                    {children}
                </div>
            </div>
            <Toaster />
...
function Menu({ name }: { name: string }) {
    ...
    return <>
        ...
        <div className="flex flex-col gap-2 mt-8">
            <Link href="/drive">
                <div className={`hover:bg-gray-100 p-2 rounded-md flex gap-2 items-center text-slate-600 hover:text-black ${url.startsWith('/drive') ? 'text-black bg-gray-100' : ''}`}>
            ...
            <Link href="/storage">
                <div className={`hover:bg-gray-100 p-2 rounded-md flex gap-2 items-center text-slate-600 hover:text-black ${url.startsWith('/storage') ? 'text-black bg-gray-100' : ''}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
            ...
            <Link href="/logout">
                <div className={`hover:bg-gray-100 p-2 rounded-md flex gap-2 items-center text-slate-600 hover:text-black ${url.startsWith('/logout') ? 'text-black bg-gray-100' : ''}`}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
